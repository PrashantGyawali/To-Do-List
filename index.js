...
function submitevent(event) {   //Prevents page from reloading when submit button clicked
...
function Datecheck(datePickerId) {
    document.getElementById(datePickerId).removeAttribute("min");
...
function sortlist(x) {
    ...
    if (x == 'datefocus') { sortDate_Priority(orderedTasks, datecondn, sortorder, priorityorder); }

    if (x == 'priorityfocus') { sortPriority_Date(orderedTasks, datecondn, sortorder, priorityorder); }
